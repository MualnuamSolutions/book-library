	...
	<div class="col-lg-5">
		{{ Form::open(array('url'=>'idcard', 'method'=>'post', 'class'=>'form-horizontal idcard-form', 'enctype'=>'multipart/form-data', 'autocomplete'=>'off')) }}
			...
			<div class="form-group {{($errors->has('name'))?'has-error':''}}">
				...
				<div class="col-sm-7">
					{{ Form::text('name', Input::old('name'), array('id'=>'student_name', 'placeholder'=>'Name of the student', 'class'=>'form-control')) }}
				</div>
			</div>
			<div class="form-group">
				{{ Form::label('student_class', 'Class', array('class'=>'col-sm-5 control-label')) }}
				<div class="col-sm-7">
					{{ Form::select('type', array('pre service'=>'Pre Service', 'in service'=>'In Service'), '', array('id'=>'student_class', 'class'=>'')) }}
				</div>
			</div>
			<div class="form-group {{($errors->has('session'))?'has-error':''}}">
				{{ Form::label('student_session', 'Session', array('class'=>'col-sm-5 control-label')) }}
				<div class="col-sm-7">
					{{ Form::text('session', Input::old('session'), array('id'=>'student_session', 'placeholder'=>'Ex: ' . date('Y') . ' - ' . (date('Y')+1), 'class'=>'form-control')) }}
				</div>
			</div>
			<div class="form-group {{($errors->has('date_of_issue'))?'has-error':''}}">
				...
				<div class="col-sm-7">
					{{ Form::text('date_of_issue', Input::old('date_of_issue'), array('placeholder'=>'Pick a date', 'class'=>'form-control', 'id'=>'student_date_of_issue')) }}
				</div>
			</div>
			<div class="form-group {{($errors->has('valid_upto'))?'has-error':''}}">
				...
				<div class="col-sm-7">
					{{ Form::text('valid_upto', Input::old('valid_upto'), array('placeholder'=>'Pick a date', 'class'=>'form-control', 'id'=>'student_validity')) }}
				</div>
			</div>
			<div class="form-group {{($errors->has('contact'))?'has-error':''}}">
				{{ Form::label('student_mobile', 'Mobile Number', array('class'=>'col-sm-5 control-label')) }}
				<div class="col-sm-7">
					{{ Form::text('contact', Input::old('contact'), array('id'=>'student_mobile', 'placeholder'=>'Student contact number', 'class'=>'form-control')) }}
				</div>
			</div>
			<div class="form-group {{($errors->has('blood_group'))?'has-error':''}}">
				{{ Form::label('student_blood_group', 'Blood Group', array('class'=>'col-sm-5 control-label')) }}
				<div class="col-sm-7">
					{{ Form::select('blood_group', array('O'=>'O', 'A'=>'A', 'B'=>'B', 'AB'=>'AB'), Input::old('blood_group'), array('id'=>'student_blood_group', 'placeholder'=>'', 'class'=>'form-control')) }}
				</div>
			</div>
			<div class="form-group {{($errors->has('present_address'))?'has-error':''}}">
				...
				<div class="col-sm-7">
					{{ Form::textarea('present_address', Input::old('present_address'), array('id'=>'student_present_address', 'placeholder'=>'Student present address', 'rows'=>3, 'class'=>'form-control')) }}
				</div>
			</div>
			<div class="form-group {{($errors->has('permanent_address'))?'has-error':''}}">
				{{ Form::label('student_permanent_address', 'Permanent Address', array('class'=>'col-sm-5 control-label')) }}
				<div class="col-sm-7">
					{{ Form::textarea('permanent_address', Input::old('permanent_address'), array('id'=>'student_permanent_address', 'placeholder'=>'Student permanent address', 'rows'=>3, 'class'=>'form-control')) }}
				</div>
			</div>
				...
				<div class="idcard-preview-box">
					...
					<div id="student_idcard" class="idcard idcard-preview">
						...
						<div class="idcard-back">
							...
							<h4>Permanent Address: <pre class="permanent-address"></pre></h4>
