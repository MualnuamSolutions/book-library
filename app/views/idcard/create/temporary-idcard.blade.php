	...
	<div class="col-lg-5">
		{{ Form::open(array('url'=>'idcard', 'method'=>'POST', 'class'=>'form-horizontal idcard-form', 'enctype'=>'multipart/form-data', 'autocomplete'=>'off')) }}
			...
			<div class="form-group {{($errors->has('tm_school'))?'has-error':''}}">
				...
				<div class="col-sm-7">
					{{ Form::text('tm_school', Input::old('tm_school'), array('id'=>'temporary_school', 'placeholder'=>'Name of school', 'class'=>'form-control')) }}
				</div>
			</div>
			<div class="form-group {{($errors->has('tm_date_of_issue'))?'has-error':''}}">
				...
				<div class="col-sm-7">
					{{ Form::text('tm_date_of_issue', Input::old('tm_date_of_issue'), array('placeholder'=>'Pick a date', 'class'=>'form-control', 'id'=>'temporary_date_of_issue')) }}
				</div>
			</div>
			<div class="form-group {{($errors->has('tm_valid_upto'))?'has-error':''}}">
				...
				<div class="col-sm-7">
					{{ Form::text('tm_valid_upto', Input::old('tm_valid_upto'), array('placeholder'=>'Pick a date', 'class'=>'form-control', 'id'=>'temporary_validity')) }}
				</div>
			</div>
			<div class="form-group {{($errors->has('tm_contact'))?'has-error':''}}">
				...
				<div class="col-sm-7">
					{{ Form::text('tm_contact', Input::old('tm_contact'), array('id'=>'temporary_mobile', 'placeholder'=>'Card holder contact number', 'class'=>'form-control')) }}
				</div>
			</div>
			<div class="form-group {{($errors->has('tm_present_address'))?'has-error':''}}">
				...
				<div class="col-sm-7">
					{{ Form::textarea('tm_present_address', Input::old('tm_present_address'), array('id'=>'temporary_present_address', 'placeholder'=>'Card holder present address', 'rows'=>3, 'class'=>'form-control')) }}
				</div>
			</div>
			<div class="form-group {{($errors->has('tm_permanent_address'))?'has-error':''}}">
				...
				<div class="col-sm-7">
					{{ Form::textarea('tm_permanent_address', Input::old('tm_permanent_address'), array('id'=>'temporary_permanent_address', 'placeholder'=>'Card holder permanent address', 'rows'=>3, 'class'=>'form-control')) }}
				</div>
			</div>
...
function temporary_generate_barcode(temporary_barcode_string) {
	$("#temporary_idcard .idcard-barcode").barcode(temporary_barcode_string, 'code128', {barHeight:18, fontSize:11});
