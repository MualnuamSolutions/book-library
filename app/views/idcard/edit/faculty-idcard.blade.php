	...
	<div class="col-lg-5">
		{{ Form::open(array('url'=>url('idcard', array($idcard->id)), 'method'=>'put', 'class'=>'form-horizontal idcard-form', 'enctype'=>'multipart/form-data', 'autocomplete'=>'off')) }}
			...
			<div class="form-group {{($errors->has('fa_name'))?'has-error':''}}">
				...
				<div class="col-sm-7">
					{{ Form::text('fa_name', Input::old('fa_name', $idcard->name), array('id'=>'faculty_name', 'placeholder'=>'Name of faculty', 'class'=>'form-control')) }}
				</div>
			</div>
			<div class="form-group {{($errors->has('fa_designation'))?'has-error':''}}">
				...
				<div class="col-sm-7">
					{{ Form::text('fa_designation', Input::old('fa_designation', $idcard->designation), array('id'=>'faculty_designation', 'placeholder'=>'Faculty designation', 'class'=>'form-control')) }}
				</div>
			</div>
			<div class="form-group {{($errors->has('fa_date_of_issue'))?'has-error':''}}">
				...
				<div class="col-sm-7">
					{{ Form::text('fa_date_of_issue', Input::old('fa_date_of_issue', date('d.m.Y', strtotime($idcard->date_of_issue)) ), array('placeholder'=>'Pick a date', 'class'=>'form-control', 'id'=>'faculty_date_of_issue')) }}
				</div>
			</div>
			...
			<div class="form-group {{($errors->has('fa_present_address'))?'has-error':''}}">
				...
				<div class="col-sm-7">
					{{ Form::textarea('fa_present_address', Input::old('fa_present_address', $idcard->present_address ), array('id'=>'faculty_present_address', 'placeholder'=>'Faculty present address', 'rows'=>3, 'class'=>'form-control')) }}
				</div>
			</div>
			<div class="form-group {{($errors->has('fa_permanent_address'))?'has-error':''}}">
				...
				<div class="col-sm-7">
					{{ Form::textarea('fa_permanent_address', Input::old('fa_permanent_address', $idcard->permanent_address ), array('id'=>'faculty_permanent_address', 'placeholder'=>'Faculty permanent address', 'rows'=>3, 'class'=>'form-control')) }}
				</div>
			</div>
...
function faculty_generate_barcode(faculty_barcode_string) {
	$("#faculty_idcard .idcard-barcode").barcode(faculty_barcode_string, 'code128', {barHeight:18, fontSize:11});
